...
(`/home/user/core/skills/sourcing-rules/SKILL.md`) before touching any
`data/*.json`, and follow the `data-edit` skill's gate for every data change:
validate → test → build → commit data + regenerated `docs/` together.** The
architecture rationale lives in `cronologia/fsp` → `docs/adrs/`.

## What this project is

A compiled static website documenting the chronology of the **reported 1936
Marian apparitions at Cimbres (Pesqueira, Pernambuco, Brazil)**, the Church's
response, and the shrine at the site. A single JSON file is the source of
truth; a zero-dependency Node script compiles it into static HTML served by
GitHub Pages.

Non-negotiable posture for this subject: apparitions are REPORTED events with
Church judgments. The dataset records who reported what and when, and what the
...
data/i18n/{es,pt}.json   Translation caches (committed). In THIS repo they are HAND-AUTHORED exact-key dictionaries (see _meta.generatedBy); keep them complete when English changes — test/i18n-completeness.test.js fails on missing or stale keys
...
The five core rules of the `sourcing-rules` skill apply verbatim. Subject
specifics for Cimbres:

1. **Reported, not asserted.** Every apparition-related entry is phrased as a
   report ("the seers reported…", "per the shrine's account…"). The site's own
   voice never affirms or denies the supernatural claim.
2. **The Church's response is the spine.** No formal 1930s–40s diocesan
   ruling has been located (RBHR 2023 academic study: consolidation "without
   major institutional interference", plus documented silencing); the first
   official act is the 2 Oct 2021 diocesan pastoral letter on the "presumíveis
   aparições". Do not upgrade that to "Vatican approval", and do not invent an
   early condemnation — record the absence, attributed.
3. **Thin sourcing is said out loud.** Much of the literature is devotional
   (the shrine's own site, Heralds of the Gospel, devotional blogs) — label
   the perspective in `references[]`, and flag any date resting on a single
   devotional source with `dateVerified: false`.
4. **Stay in scope.** The Xukuru people are the territorial context (one
   dated event + one organization entry); Xukuru history at large — including
   the 1998 assassination of Chicão Xukuru — is NOT this repo's subject.
5. **Known traps** are in `KEYWORDS.md` (title collision with the Miraculous
   Medal; the unverified "11 February 1937" end date; the seer's name variant
   "Maria da Luz Gonçalves Teixeira"; Dom Adelmo Cavalcante Machado's
   assumed-but-undocumented role).
